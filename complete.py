...
def fix_mistake(str_input, fixed_list):
    ...
    if len(fixed_list) < 5:
        ...
        fixed_list += list(sorted(add_and_check_dup(fixed_list, buf, scores - 2)[:5 - len(fixed_list)],
                                  key=lambda x: x.completed_sentence))[:5 - len(fixed_list)]
        if len(fixed_list) < 5:
            # 3scores
            ...
            if len(fixed_list) < 5:
                ...
                fixed_list += list(sorted(add_and_check_dup(fixed_list, buf, scores - 4)[:5 - len(fixed_list)],
                                          key=lambda x: x.completed_sentence))[:5 - len(fixed_list)]
                if len(fixed_list) < 5:
                    # 5scores
                    fixed_list += list(sorted(add_and_check_dup(fixed_list, swap_char(str_input, 0, 1), scores - 5),
                                              key=lambda x: x.completed_sentence))[:5 - len(fixed_list)]
                    if len(fixed_list) < 5:
                        # 6scores
                        fixed_list += list(
...
def get_best_k_completions(input_to_search):
    ...
    try:
        if len(completion_data_struct.substrings_dict[input_to_search]) < 5:
            raise Exception
        return add_and_check_dup([], completion_data_struct.get_list_of_k_best(input_to_search),
                                 len(input_to_search) * 2)
    except Exception:
        ls = list(sorted(
        ...
        return fix_mistake(input_to_search, ls)
